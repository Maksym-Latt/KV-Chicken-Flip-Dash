...
class GameEngine {
    ...
    fun update(state: GameState, deltaTime: Long): GameState {
        ...
        // 7. Spawning
        val (finalPlatforms, finalEggs) = spawnNewEntities(nextPlatforms, remainingEggs)

        // 8. Cooldown Update
        val nextCooldown = (state.jumpCooldownMs - deltaTime).coerceAtLeast(0f)
        ...
        return state.copy(
            ...
            jumpCooldownMs = nextCooldown
        )
    }

    fun flipGravity(state: GameState): GameState {
        if (state.isGameOver || state.isPaused || state.jumpCooldownMs > 0) return state
        return state.copy(
            chicken =
                state.chicken.copy(
            ...
            jumpCooldownMs = state.maxJumpCooldownMs
        )
    }
    ...
    private fun spawnNewEntities(
        ...
        if (lastX < 3000f) {
            ...
            if (isPit) {
                ...
                spawnReliefCluster(mutPlat, lastX - 200f, islandY, 600f, true)
            } else {
                ...
                if (Random.nextInt(4) < 3) {
                    val islandY =
                        (Random.nextFloat() * 120f + 50f).coerceIn(
                            50f,
                            200f
                        )
                    spawnReliefCluster(
            ...
            if (Random.nextFloat() < 0.6f) {
                val latestPlat = mutPlat.lastOrNull()
                if (latestPlat != null) {
                    val eggY =
                        if (latestPlat.isCeiling) {
                            latestPlat.position.y +
                                    latestPlat.size.height +
                                    30f
                        } else {
                            latestPlat.position.y - 70f
                        }
                    mutEggs.add(
                        Egg(
                            Offset(
                                latestPlat.position.x +
                                        latestPlat.size.width *
                                        0.3f,
                                eggY
                            )
                        )
    ...
    private fun spawnReliefCluster(
        ...
        for (i in 0 until segmentCount) {
            ...
            if (segmentCount > 1 && i == 0) {
                // For islands, favor growing UP (-Y) to keep a flat bottom and
                // avoid getting too thin
                val step =
                    if (isIsland) -100f
                    else (if (Random.nextBoolean()) 100f else -100f)
                currentY += step
                currentY =
                    currentY.coerceIn(
